...
library(tidyverse) ; library(httr) ; library(readxl) ;   library(lubridate) ; library(reactable) ; library(zoo) ; library(ggtext) ; library(scales) ; library(shiny) ; library(ggiraph) ; library(ggrepel) ; library(sf) ; library(classInt) ; library(leaflet) ; library(htmlwidgets) ; library(htmltools)
...
weekly_cases <- read_xlsx(tmp, sheet = 2, skip = 8) %>% 
...
msoa_cases <- left_join(msoa, weekly_cases, by = "msoa11cd") %>% 
  mutate(popup = str_c("<strong>", msoa11hclnm, "</strong><br/><em>", lad19nm, "</em><br/><strong>", n, "</strong> cases<br/>", format(unique(.$date)-days(6), "%d %B"), " - ", format(unique(.$date), "%d %B")) %>% map(HTML))

# calculate class intervals for choropleth map
breaks <- classIntervals(msoa_cases[msoa_cases$n > 0, ]$n, n = 5, style = "jenks")$brks
pal <- colorBin(palette = "PuBu", domain = NULL, bins = breaks, na.color = "#D7D8D7")

# title
div(class = "subtitle",
        div(class = "title", "Confirmed coronavirus cases by local area"),
        paste("Greater Manchester,", format(unique(msoa_cases$date)-days(6), "%d %B"), " - ", format(unique(msoa_cases$date), "%d %B")))

# create map
leaflet(data = filter(msoa_cases, n != 0), width = "100%") %>%
  ...
  addPolygons(data = filter(msoa_cases, n == 0), fillColor = "#FFFFFF", fillOpacity = 0.8, smoothFactor = 1, stroke = TRUE, weight = 0.5, color = "#bdbdbd", opacity = 1, label = ~popup, labelOptions = labelOptions(style = list("font-weight" = "normal", padding = "3px 8px"), textsize = "13px", direction = "auto"), highlightOptions = highlightOptions(color = "#212121", weight = 2, bringToFront = TRUE)) %>% 
  addPolygons(fillColor = ~pal(n), fillOpacity = 0.5, smoothFactor = 0.8, stroke = TRUE, weight = 1, color = "#bdbdbd", opacity = 1, label = ~popup, labelOptions = labelOptions(style = list("font-weight" = "normal", padding = "3px 8px"), textsize = "13px", direction = "auto"), highlightOptions = highlightOptions(color = "#212121", weight = 2, bringToFront = TRUE)) %>%
  addLegend(pal = pal, values = ~n, opacity = 0.7, title = "Number of cases", position = "bottomright") %>%
  onRender(
