...
# Create lookup dataframe for Trafford MSOAs and their House of Commons names, ready to use as a join
...
positivity_rate <- fromJSON(content(GET(url = 'https://api.coronavirus.data.gov.uk/v1/data?filters=areaType=ltla;areaName=trafford&structure={"date":"date","uniqueCasePositivityBySpecimenDateRollingSum":"uniqueCasePositivityBySpecimenDateRollingSum"}', timeout(50)), as = "text"), flatten = TRUE) %>% 
  pluck("data") %>% 
  as_tibble() %>% 
