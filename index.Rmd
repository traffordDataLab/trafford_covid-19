...
try(
    {
      ...
      msoa_cases <- left_join(msoa, msoa_cases, by = "msoa11cd") %>% 
        mutate(popup = ifelse(n == 0,
                              ...
                              str_c(msoa11hclnm, ":<br />", n, " cases, ", rate, " per 100,000") %>%
                              map(HTML)))
...
if (nrow(msoa_cases) > 0) {
  ...
  mapCases <- leaflet(data = msoa_cases, width = "100%", options = leafletOptions(zoomControl = FALSE)) %>%
    ...
    addTiles(urlTemplate = "", attribution = '<a href="https://www.ons.gov.uk/methodology/geography/licences">Contains OS data © Crown copyright and database right (2022)</a>', options = tileOptions(minZoom = 11, maxZoom = 11)) %>%
