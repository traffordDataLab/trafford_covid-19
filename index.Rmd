...
<p>Source: <a href="https://coronavirus.data.gov.uk/details/download" target="_blank">UK Coronavirus Dashboard</a> developed by <a href="https://www.gov.uk/government/organisations/public-health-england" target="_blank">Public Health England</a></p>
<br />
<div class="container-fluid">
<div class="row">
<div class = "col-sm-6">
```{r, dev = 'svg', fig.alt = str_c(slice_max(rolling_cases, order_by = date)$new_cases, ' confirmed cases on ', format(max(rolling_cases$date), '%d %B'))}
...
<div class = "col-sm-6">
```{r, dev = 'svg', fig.alt = str_c(slice_max(total_cases, order_by = date)$cum_cases, ' total confirmed cases up to ', format(max(total_cases$date), '%d %B'))}
...
<div class = "col-sm-6">
```{r, dev = "svg", fig.alt = str_c(alt_text_age_bands$ageband[1], ': ', alt_text_age_bands$roll_avg_rounded[1], ', ', alt_text_age_bands$ageband[2], ': ', alt_text_age_bands$roll_avg_rounded[2], ', ', alt_text_age_bands$ageband[3], ': ', alt_text_age_bands$roll_avg_rounded[3], ', ', alt_text_age_bands$ageband[4], ': ', alt_text_age_bands$roll_avg_rounded[4], ', ', alt_text_age_bands$ageband[5], ': ', alt_text_age_bands$roll_avg_rounded[5], ', ', alt_text_age_bands$ageband[6], ': ', alt_text_age_bands$roll_avg_rounded[6], ' (7 day avg per 100K) at ', format(alt_text_age_bands$date[1], '%d %B'))}
###--------- This is for the plot img's alt text - simplifies the code in the chunk header above
# get the cases rates for the latest date and round them to 1dp
alt_text_age_bands <- age_specific_cases %>%
...
<div class = "row">
<div class = "col-sm-6">
```{r, dev = "svg", fig.alt = if (!is.null(positivity_rate)) { str_c(slice_max(positivity_rate, order_by = date)$uniqueCasePositivityBySpecimenDateRollingSum, '% of tests positive (7 day average) as at ', format(max(positivity_rate$date), '%d %B')) } }
# Positivity rate
div(class = "subtitle",
...
<div class = "col-sm-6">
```{r, dev = "svg", fig.alt = str_c('Approximately ', alt_text_hospital_cases$ma_cases_rounded, ' patients (7-day rolling average) as at ', format(max(alt_text_hospital_cases$date), '%d %B'))}
###--------- This is for the plot img's alt text - simplifies the code in the chunk header above
# select the latest date that we have an average case number and then round to 0dp
alt_text_hospital_cases <- hospital_cases %>%
  drop_na() %>%
  filter(date == max(date)) %>%
