...
# Using last 7 days of complete cases
...
<!--          popup = str_c("<strong>", msoa11hclnm, "</strong><br/><em>", lad19nm, "</em><br/><strong>", rate, "</strong> cases per 100,000") %>% map(HTML)) -->
...
<!--         paste("Greater Manchester, week ending ", format(Sys.Date() - days(5), "%d %B"))) -->
<!-- # create map -->
...
daily_incidence <- cases %>% 
  # exclude incomplete cases
  filter(date <= max(date)-days(5)) %>% 
  arrange(area_name, date) %>%
  left_join(population, by = "area_code") %>% 
  mutate(rate = round(new_cases/population*100000,1), 
         seven_day = round(rollmean(rate, 7, align = "right", fill = NA),1),
         fourteen_day = round(rollmean(rate, 14, align = "right", fill = NA),1)) %>% 
  filter(date == max(date)) %>% 
  select(area_name, seven_day, fourteen_day) %>% 
...
tbl <- reactable(daily_incidence,
                 ...
                 columns = list(
                   ...
                   seven_day = colDef(name = "7-day moving average", align = "left"),
                   fourteen_day = colDef(name = "14-day moving average", align = "left")
                   )
                 )

div(class = "div",
    div(class = "subtitle",
        div(class = "title", "Daily incidence per 100,000 population"),
        paste("to", format(max(cases$date)-days(5), '%d %B %Y'))
        ),
    tbl,
    div(class = "caption", p("Source: ", a("Public Health England", href = 'https://coronavirus.data.gov.uk/', target = "_blank")))
    )
...
ggplot() +
  ...
  scale_x_continuous(expand = c(0.005, 0.005),
                     breaks = seq(0, max(weekly_cases$rate), 2),
                     labels = seq(0, max(weekly_cases$rate), 2),
