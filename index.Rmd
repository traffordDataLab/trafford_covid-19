...
# To reduce the amount of data being provided by the API we can request just data for Trafford by adding areaCode=E08000009 into the URL.
# However, we now get duplicates for each record.  Therefore we need to de-dupe the results as well as filter for the latest date.
# Also note: not all MSOAs have data due to low number suppression.
...
local_cases <- left_join(msoa, msoa_cases, by = "msoa11cd") %>% 
  mutate(popup = ifelse(n == 0,
                        str_c(msoa11hclnm, ": Data suppressed") %>% map(HTML),
                        str_c(msoa11hclnm, ": ", n, " cases, ", rate, " per 100,000") %>% map(HTML)))
